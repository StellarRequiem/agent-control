...
| agent-soc watch | **Yes (default)** | Freeze-only (`AGENT_SOC_AUTO_DISARM=0`); no auto-disarm |
| Auto-post / invent confirm | **Never** | Human high-blast only |

## Install

```bash
# Bridges + freeze-only SOC watch (default)
bash ~/agent-control/scripts/always_available.sh install

# Bridges only (no continuous watch)
INSTALL_SOC_WATCH=0 bash ~/agent-control/scripts/always_available.sh install
